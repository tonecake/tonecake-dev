...
// return Array
Harmony.prototype.getChord.compact = function( key, quality, seventh, scale )
{
    var context = new Harmony();
    var chord = context.getChord( key, quality, seventh, scale );

    return[ chord.root, chord.third, chord.fifth, chord.seventh ];
};

// return number(int) 이 method도 외부 객체에 대한 의존성을 버리도록 하세요.
Harmony.prototype.getInterval = function( foo, bar )
{
    ...
    for( var i=0; i<this.rule.notes.length; i++ )
    {
        ...
        {
            if( this.rule.notes[i].name[j] === foo )
            {
                indexFoo = this.rule.notes[i].index; // or i
            }
        }
        for( var j=0; j<this.rule.notes[i].name.length; j++ )
        {
            if( this.rule.notes[i].name[j] === bar )
            {
                indexBar = this.rule.notes[i].index; // or i
            }
        }
    }
...
Harmony.prototype.calculateInterval = function( index, interval, direction )
{
    ...
    if( typeof interval === 'string' )
    {
        ...
        var result, kindOfInterval,
            ...
            interval =  parseInt(interval.substr(interval.length-1, interval.length)) - 1;

        if( interval === 0 || interval === 3 || interval === 4 || interval === 8 )
        {
            kindOfInterval = 'perfect';
        }
        else if(interval === 1 || interval === 2 || interval === 5 || interval === 6 )
        {
